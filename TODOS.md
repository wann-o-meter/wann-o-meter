...
- [x] Timeline shows „möglich ab" capsules for some tasks; the cards never show a start date. Same data, two views, one of them silently dropping it.

## Consistency

- [x] Three date formats on one screen: `Do., 03.09.2026` (summary), `Di, 04.08.2026` (card), `Fr, 16. Oktober 2026` (flag). Pick one long form and one short form and define where each is used.
...
- [x] Only two of ten markers have a „möglich ab" capsule. Verify this is real data and not a rendering condition that fails for tasks without an explicit start.
- [x] Werktag and Wochenende cells in the day band are nearly the same value. The band's whole job is making weekday structure visible; widen the gap between those two fills.
- [x] Month labels sit under the tick rather than centred in the month span, so „Aug" appears roughly above 20 August.
- [x] The VORHABEN field is styled as a card like UMZUGSTAG and ORT but isn't interactive. Either make it a select (it's the natural place to switch Vorhaben) or demote it to a plain line of text.
- [x] The page title already says _Umzug_, and the VORHABEN field repeats _Umzug innerhalb Deutschlands_. One of them can go.
...
- [x] The legend row mixes two unrelated things: state keys (offen / erledigt / möglich ab) and band filters (Feiertage / Schulferien checkboxes). Separate them; filters are controls, keys are not.
- [x] Overdue red appears in the timeline and on the rail but has no legend entry, despite being the most consequential state on this particular screen.
- [x] Markers stack three deep at one x while long stretches sit empty. Worth checking whether lane packing is using the full occupied width including capsules, or only marker radius.
...
- [x] `▶` as a disclosure marker should be a real `<details>`/`<summary>` (or a button with `aria-expanded`), not a glyph.
- [x] The Umzugstag label _„Fr, 16. Oktober 2026"_ runs to the container edge and will clip at narrower widths — needs the flip-to-`text-anchor: end` clamp.
- [x] Check contrast of the grey month labels and legend text against the light blue background; they look under 4.5:1.
- [x] If the header is sticky, confirm the _AUFGABEN_ section and each card have `scroll-margin-top` so marker → card jumps don't land underneath it.
...
These were already true in the code when the list was written, so they are
checked off with what was checked rather than with a fix:

- „möglich ab" already shows on the cards, next to the Frist.
- Only two tasks carry `earliest_offset_days` in `data/umzug/_bundesweit.yaml`,
  so two capsules is the real data, not a rendering condition.
- Month labels are centred over the month span (`mid` in `Timeline.vue`).
- The disclosures are real `<details>`/`<summary>`, never a `▶` glyph.
- The Umzugstag label already flips to `text-anchor: end` near the right edge.
- `--muted` on `--paper` is 6.8:1 in light and 8.4:1 in dark, both over 4.5:1.
- `packLanes` receives the full occupied span, capsule included, not the
  marker radius.
