...
def _stage_candidate(
    ...
    url: str = "https://example.invalid/events",
) -> dict:
    doc_hash = staging.write_document(source_id, run_ts, url, "text/calendar", b"BEGIN:VCALENDAR\r\nEND:VCALENDAR\r\n")
    candidate = staging.build_candidate(
        source_id, subject_slug, event, doc_hash, subject_name=subject_name, category=category
    )
...
class TestReviewApprove:
    def test_approve_writes_the_event_and_records_the_decision(self, client):
        candidate = _stage_candidate(
            "test-source", "20260724-000000", "hechingen", VALID_EVENT, category="veranstaltungen"
        )
        ...
        file = yaml.safe_load(data_path.read_text(encoding="utf-8"))
        assert file["windows"][0]["name"] == "Stadtfest"
    ...
    def test_an_approved_candidate_leaves_the_queue(self, client):
        ...
        file = yaml.safe_load(data_path.read_text(encoding="utf-8"))
        file["windows"] = []
        data_path.write_text(yaml.dump(file), encoding="utf-8")
...
class TestReviewModify:
    def test_modify_writes_the_corrected_event_and_records_the_correction(self, client):
        ...
        assert response.status_code == 302
        file = yaml.safe_load(
            (service.DATA_ROOT / "veranstaltungen" / "hechingen" / "data.yaml").read_text(encoding="utf-8")
        )
        assert file["windows"][0]["name"] == "Stadtfest Hechingen"
        assert file["windows"][0]["to"] == "2026-08-16"
...
class TestBulkApprove:
    ...
    def test_many_identically_named_events_each_land_as_their_own_window(self, client):
        ...
        candidates = [
            _stage_candidate("test-source", "20260724-000000", "sofi", _eclipse_event(date)) for date in dates
        ]
        ...
        assert response.status_code == 200
        file = yaml.safe_load((service.DATA_ROOT / "sofi" / "sofi" / "data.yaml").read_text(encoding="utf-8"))
        assert sorted(w["from"] for w in file["windows"]) == dates
...
class TestGermanAndRangeHighlighting:
    """kmk.org writes school holidays as "26.10. - 30.10." - day.month, a
    trailing dot, no year, and the two ends joined into one string. Candidates
    hold ISO dates, so before these variants existed the KMK document
    highlighted nothing at all and the whole select-in-place review was dead
    for that source."""

    def test_a_german_day_month_date_is_found(self):
        result = service._highlight_dates("Ferien am 31.10. dieses Jahr", ["2026-10-31"])
        assert "<mark>31.10.</mark>" in result

    def test_a_german_full_date_is_found(self):
        result = service._highlight_dates("Ferien am 31.10.2026", ["2026-10-31"])
        assert "<mark>31.10.2026</mark>" in result

    def test_a_range_highlights_as_one_hit_not_two(self):
        """Two marks on one window would read as two things to decide."""
        result = service._highlight_dates("BW 26.10. - 30.10. Herbst", ["2026-10-26", "2026-10-30"])
        assert "<mark>26.10. - 30.10.</mark>" in result
        assert result.count("<mark>") == 1

    def test_a_range_spanning_the_year_boundary_still_matches(self):
        result = service._highlight_dates("Weihnachten 23.12. - 09.01. dann", ["2026-12-23", "2027-01-09"])
        assert "<mark>23.12. - 09.01.</mark>" in result

    def test_an_iso_range_is_still_one_hit(self):
        result = service._highlight_dates("von 2026-08-15 - 2026-08-17", ["2026-08-15", "2026-08-17"])
        assert "<mark>2026-08-15 - 2026-08-17</mark>" in result

    def test_a_range_becomes_a_single_checkbox_in_the_document_view(self):
        candidate = {
            "candidate_id": "kmk:abc",
            "event": {"from": "2026-10-26", "to": "2026-10-30", "name": "Herbstferien"},
        }
        result = service._highlight_candidates("BW 26.10. - 30.10. x", [candidate], "schulferien_kmk")

        assert "<span>26.10. - 30.10.</span>" in result
        assert result.count('type="checkbox"') == 1

    def test_a_single_day_window_has_no_range_spelling(self):
        assert service._date_range_variants("2026-10-31", "2026-10-31") == []
...
class TestReviewCandidateDetailHighlighting:
    def test_an_html_document_snapshot_is_shown_with_the_candidates_date_highlighted(self, client):
        doc_hash = staging.write_document(
            ...
            "text/html",
            b"<html><body><p>Stadtfest am 2026-08-15 in Hechingen</p></body></html>",
        )
...
class TestCreateCrawlSource:
    ...
    def test_two_bare_urls_on_the_same_domain_derive_distinct_ids_via_path(self, client, tmp_path):
        first = client.post(
            "/crawl-sources/new", data={"seed_url": "https://eclipse.gsfc.nasa.gov/solar/"}, follow_redirects=False
        )
        second = client.post(
            "/crawl-sources/new", data={"seed_url": "https://eclipse.gsfc.nasa.gov/lunar/"}, follow_redirects=False
        )
...
class TestEditCrawlSource:
    ...
    def _approve(self, client, source_id, subject_slug, category, date, url="https://example.invalid/events"):
        ...
        client.post(
            ...
            data={"category": category, "license": "tos_checked"},
            follow_redirects=False,
        )
        return candidate
    ...
    def test_moves_the_page_folder_and_carries_its_page_yaml_over(self, client):
        ...
        file = yaml.safe_load((new_folder / "data.yaml").read_text(encoding="utf-8"))
        assert [w["from"] for w in file["windows"]] == ["2026-08-12"]
        assert file["subject"] == {"slug": "sonnenfinsternis", "category": "astronomie"}
    ...
    def test_merging_into_an_existing_page_unions_windows_and_citations(self, client):
        ...
        self._approve(
            client, "nasa-a", "sonnenfinsternis", "astronomie", "2026-08-12", url="https://example.org/2001-2100"
        )
        ...
        file = yaml.safe_load(
            (service.DATA_ROOT / "astronomie" / "sonnenfinsternis" / "data.yaml").read_text(encoding="utf-8")
        )
        assert sorted(w["from"] for w in file["windows"]) == ["1999-08-11", "2026-08-12"]
        assert len(file["source"]) == 2

    def test_the_same_window_from_two_sources_keeps_one_entry_with_both_citations(self, client):
        """store.merge_windows's reason for existing - two sources agreeing
        ...
        assert len(file["windows"][0]["source_urls"]) == 2
    ...
    def test_rejections_follow_the_source_to_its_new_page(self, client):
        ...
        self._source(client, "nasa-a", "astronomie", "nasa-a")
        candidate = _stage_candidate(
            "nasa-a", "20260724-000000", "nasa-a", _eclipse_event("2026-08-12"), category="astronomie"
        )
    ...
    def test_preserves_fields_the_form_does_not_edit(self, client):
        ...
        client.post(
            "/crawl-sources/stuttgart-veranstaltungen/edit",
            data={
                "category": "veranstaltungen",
                "subject_slug": "stadtfeste",
            },
            follow_redirects=False,
        )
    ...
    def test_switching_extraction_mode_persists_and_omitting_it_leaves_it_alone(self, client):
        ...
        client.post("/crawl-sources/nasa-a/edit", data={**base, "extraction_mode": "static"}, follow_redirects=False)
    ...
    def test_404s_for_an_unknown_source(self, client):
        response = client.post(
            "/crawl-sources/does-not-exist/edit",
            data={
                "category": "astronomie",
                "subject_slug": "x",
            },
...
class TestSuggestTags:
    ...
    def _page(self, client, tags=None):
        ...
        (folder / "page.yaml").write_text(
            yaml.dump({"title": "Sonnenfinsternis", "tags": tags or []}), encoding="utf-8"
        )
        (folder / "data.yaml").write_text(
            yaml.dump(
                {"subject": {"slug": "sonnenfinsternis"}, "windows": [_eclipse_event("2026-08-12")], "source": []}
            ),
            encoding="utf-8",
        )
    ...
    def test_the_existing_vocabulary_is_offered_for_reuse(self, client, monkeypatch):
        ...
        seen = {}
        monkeypatch.setattr(
            service, "suggest_tags", lambda text, title, existing, **kw: seen.setdefault("existing", existing) or []
        )
...
class TestEditPageKeepsEveryCitation:
            ...
            yaml.dump(
                {
                    ...
                    "source": [
                        {
                            "url": "https://a.invalid/cat",
                            "license": "tos_checked",
                            "retrieved_at": "2026-07-25",
                            "extraction": "llm",
                        },
                        {
                            "url": "https://b.invalid/cat",
                            "license": "tos_checked",
                            "retrieved_at": "2026-07-25",
                            "extraction": "llm",
                        },
    ...
    def test_changing_the_license_keeps_every_source_and_restamps_them_all(self, client):
        ...
        response = client.post(
            "/pages/astronomie/sonnenfinsternis/edit",
            data={
        ...
        assert response.status_code == 302
        file = yaml.safe_load((folder / "data.yaml").read_text(encoding="utf-8"))
        assert [s["url"] for s in file["source"]] == ["https://a.invalid/cat", "https://b.invalid/cat"]
        assert {s["license"] for s in file["source"]} == {"official_par5"}
    ...
    def test_every_cited_url_still_has_a_source_entry_afterwards(self, client):
        ...
        file = yaml.safe_load((folder / "data.yaml").read_text(encoding="utf-8"))
        cited = {u for w in file["windows"] for u in (w.get("source_urls") or [])}
        assert cited <= {s["url"] for s in file["source"]}
