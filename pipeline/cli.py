...
def _cmd_sources(args: argparse.Namespace) -> int:
    ...
    from core.runner import load_sources_config

    crawl = crawl_config.load_all_crawl_sources()
    batch = load_sources_config()
    ...
    rows += [(sid, "batch", c.get("category", ""), c.get("url", "")) for sid, c in batch.items()]
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    """
    from core import crawl_config
    from core.runner import load_sources_config, parse_params
    from core.runner import run as run_batch

    if args.source_id in load_sources_config():
        try:
    ...
    if args.source_id not in crawl:
        known = sorted([*crawl, *load_sources_config()])
