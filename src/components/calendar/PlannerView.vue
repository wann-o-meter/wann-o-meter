...
const monthLabel = computed(
  () => `${MONTH_NAMES[props.monthIndex0]} ${props.year}`,
);
...
const rows = computed(() =>
  Array.from({ length: daysInMonth(props.year, props.monthIndex0) }, (_, i) => {
    ...
    return {
      ...
      cells: visibleLayers.value.length
        ? visibleLayers.value.map((l) => cellFor(l, iso, day === 1))
        : [null],
    };
  }),
);
    ...
    <div class="planner-scroll">
      <div class="planner" :style="{ gridTemplateColumns: columns }">
        ...
        <template v-for="row in rows" :key="row.iso">
          <span
            class="day"
            :class="{ weekend: row.weekend, today: row.iso === todayIso }"
          >
          ...
          >
            <a v-if="cell?.label" :href="cell.url" class="slot-label">{{
              cell.label
            }}</a>
          </span>
        </template>
