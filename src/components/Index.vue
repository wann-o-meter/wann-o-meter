  ...
  <div class="wom-timeline">
    ...
    <div class="stage">
        ...
        <div class="track" ref="trackEl" :style="{ width: trackWidth + 'px' }">
          ...
          <template v-if="placed">
            ...
            <button
              ...
              :style="{ left: pxIso(t.date) + 'px', viewTransitionName: `umzug-item-${t.id}` }"
              :data-label="nodeLabel(t)"
              :aria-label="nodeLabel(t)"
            ></button>
            <div
              class="pin"
              :style="{ left: pxIso(anchorDate) + 'px', viewTransitionName: `umzug-item-${ANCHOR_ID}` }"
            >
      ...
      <section v-if="placed" class="result on">
        ...
        <div class="acts">
          <a class="btn primary" :href="planHref">Zeitplan öffnen</a>
...
const props = defineProps<{
  ort: string;
  slug: string; // matches DeadlinePlanner's PlanVariant.slug and its ?variant= query param
  regionCode?: string;
  deadlines: Deadline[];
}>();
...
// Real navigation to the full planner, carrying the picked date and Kommune
// along so it opens on the exact plan just shown here - the two ?date=/
// ?variant= params DeadlinePlanner already reads for a shareable link.
const planHref = computed(() => {
  const params = new URLSearchParams();
  if (anchorDate.value) params.set("date", anchorDate.value);
  params.set("variant", props.slug);
