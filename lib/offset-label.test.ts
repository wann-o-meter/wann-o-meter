...
const base: Deadline = { id: "x", label: "X", offset_days: 0, source_url: null };
const d = (over: Partial<Deadline>): Deadline => ({ ...base, ...over });

describe("offsetLabel", () => {
  it("scales the unit to the distance", () => {
    expect(offsetLabel(d({ offset_days: -120 }), "Umzugstag")).toBe("4 Monate vorher");
  ...
  it("never invents a number for an unresearched deadline", () => {
    expect(offsetLabel(d({ offset_days: null }), "Umzugstag")).toBe("Frist noch nicht recherchiert");
  });
});

describe("sourceLabel", () => {
  it("distinguishes missing from not-applicable", () => {
    expect(sourceLabel(d({}))).toBe("Quelle fehlt");
    expect(sourceLabel(d({ no_source_needed: true }))).toBe("Keine gesetzliche Frist");
    expect(sourceLabel(d({ source_url: "https://e.de", source_label: "§ 1" }))).toBe("§ 1");
  });
});

describe("byOffset", () => {
  it("sorts unresearched deadlines last", () => {
    const sorted = [d({ offset_days: null, id: "u" }), d({ offset_days: 5, id: "b" }), d({ offset_days: -5, id: "a" })]
      .sort(byOffset)
      .map((x) => x.id);
